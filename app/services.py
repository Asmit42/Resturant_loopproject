...
from app.sqlconnection import db
import threading
import time
import functools
import pandas as pd
...
def import_data():
    ...
    #    getting data from csv file
        stores_csv = pd.read_csv("collectionofdata/stores.csv", chunksize=batch_size)
        business_hours_csv = pd.read_csv("collectionofdata/business_hours.csv", chunksize=batch_size)
        timezones_csv = pd.read_csv("collectionofdata/timezones.csv", chunksize=batch_size)
        timezone_dict = {}
        for _, row in pd.concat(timezones_csv).iterrows():
            timezone_dict[row['store_id']] = pytz.timezone(row['timezone_str'])

    #  making data storing into database
...
def refresh_cache():
    while True:
        ...
        run_import_data()  # refilling it again
